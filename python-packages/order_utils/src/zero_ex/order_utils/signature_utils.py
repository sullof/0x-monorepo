...
def is_valid_signature(
    provider, data: str, signature: str, signer_address: str
) -> Tuple[bool, str]:
    # docstring considered all one line by pylint: disable=line-too-long
    """Check the validity of the supplied elliptic curve signature.

    Check if the supplied elliptic curve signature corresponds to signing
    :param data: with the private key corresponding to :param signer_address:.

    :param data: The hex encoded data signed by the supplied signature.
    :param signature: An object containing the elliptic curve signature
        parameters.
    :param signer_address: The hex encoded address that signed the data,
        producing the supplied signature.
    :rtype: Boolean indicating whether the ECSignature is valid.
